...
def test_channel_private():
    ...
    with pytest.raises(InputError) as e:
        channel.channel_addowner(chicken['token'], -9999, bob['u_id'])
...
def test_channel_invite_exceptions():
    ...
    assert str(e.value) == '400 Bad Request: user_id does not exist'
...
def test_channel_messages_working():
    ...
    for _ in range(20):
        message_send(user_1['token'], new_channel['channel_id'], "Hello")
    ...
    for _ in range(100):
        message_send(user_1['token'], new_channel['channel_id'], "Goodbye")
